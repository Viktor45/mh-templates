...
Важно:
- Убедитесь, что у вас в подписках есть хотя бы несколько рабочих российских прокси дня надежности соединения, иначе автоматика направит трафик сначала через иностранный прокси, а потом когда они кончатся - напрямую, без прокси.
- Если сомневаетесь — добавьте несколько разных подписок через `SUB1`, `SUB2` и т.д.
- Если необходимо жестко закрепить за каким-либо сервисом страну, чтобы она не поменялась в результате работы автоматики - выставите ее через `MANUAL` переключатель на сервис и выберите для сервиса переключатель `MANUAL`, либо самостоятельно пропишите дополнительный [фильтр](https://wiki.metacubex.one/en/config/proxy-groups/#filter) внутри `stargate-chain.yaml`.
