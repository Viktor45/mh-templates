...
При первом запуске обязательно дождитесь прогрузки подписок и настройте `SELECTOR` и `MANUAL` / `RU_MANUAL` вручную — выберите нужные прокси.

NB: Заметил, что когда UI-панель управления проксями закрыта и нигде не висит активной в браузере - нагрузка на память снижается на ~10MB, поэтому рекомендуется не оставлять ее нигде просто так, если ничего не настраиваете.
...
Как починить?
— Нажмите правой кнопкой мыши на название сбитого переключателя или на значок скорости, если у вас не zashboard — логика автоматически восстановится.
